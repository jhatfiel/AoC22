...
p.onClose = () => {
    ...
    const partial = totalWeight/3;
    console.assert(partial === Math.floor(partial), `Weight [${totalWeight}] should be evenly divisible by 3`);
        ...
        if (group1.size <= minGroup1) {
            ...
            group2Choices.forEach((group2Mask) => {
                ...
                group3Choices.forEach((group3Mask) => {
                    ...
                    //console.log(`${group1.join(' ').padEnd(30)} / ${group2.join(' ').padEnd(30)} / ${group3.join(' ').padEnd(30)}`);
                    if (group1.size < minGroup1) {
