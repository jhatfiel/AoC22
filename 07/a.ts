...
rl.on('close', () => {
    var matchDirectories = c.root.filter((e) => {
        return e.getSize() <= 100000;
    })
    /*
    matchDirectories.forEach((e) => {
        console.log('Directory ' + e.name + ' size is ' + e.getSize());
    })
    */

    var matchSize = matchDirectories.reduce((acc, e) => acc + e.getSize(), 0);
    console.log('sum of matching sizes = ' + matchSize);
